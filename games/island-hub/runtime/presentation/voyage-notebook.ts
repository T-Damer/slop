...
import { voyageItems, voyageRegions, voyageResidents, type VoyageResidentId, type VoyageState, type VoyageRegionId } from '../domain/voyage-registry.ts';
import { voyageLayout } from '../domain/voyage-layout.ts';
...
export function notebookMarkup(state: VoyageState, world: IslandBlueprint, player: IslandPoint, tab: VoyageTab): string {
  const content = tab === 'map' ? mapMarkup(state, world, player) : tab === 'finds' ? findsMarkup(state, world) : questsMarkup(state);
...
export function dockMarkup(state: VoyageState): string {
  return `<header><div><small>Лодка у причала</small><h2>Куда отправимся?</h2></div>${closeButton()}</header>
    ...
    <div class="voyage-pages">${(Object.keys(voyageRegions) as VoyageRegionId[]).filter((id) => id !== state.region).map((id) =>
...
function findsMarkup(state: VoyageState, world: IslandBlueprint): string {
  const totalSites = (Object.keys(voyageRegions) as VoyageRegionId[])
    .reduce((sum, id) => sum + voyageLayout(id, world).sites.length, 0);
  return `<div class="voyage-pockets">${(Object.keys(voyageItems) as (keyof typeof voyageItems)[]).map((id) =>
    ...
    <h3>Память о прогулках</h3><p>Острова: ${state.visited.length}/${Object.keys(voyageRegions).length} · Открытые места: ${state.discovered.length}/${totalSites}</p>
...
function mapMarkup(state: VoyageState, world: IslandBlueprint, player: IslandPoint): string {
  ...
  const river = layout.river ? `<polyline points="${layout.river.points.map((p) => `${p.x},${p.z}`).join(' ')}" fill="none" stroke="#78b8c1" stroke-linecap="round" stroke-linejoin="round" stroke-width="${layout.river.width}"/>` : '';
  const bridges = layout.bridges.map((entry) => {
    const dx = Math.cos(entry.rotation) * entry.length / 2; const dz = -Math.sin(entry.rotation) * entry.length / 2;
    return `<line x1="${entry.x - dx}" y1="${entry.z - dz}" x2="${entry.x + dx}" y2="${entry.z + dz}" stroke="#a77850" stroke-width="${entry.width}" stroke-linecap="round"/>`;
  }).join('');
  return `<svg class="voyage-map" viewBox="${-radius} ${-radius} ${radius * 2} ${radius * 2}" role="img" aria-label="Карта острова: север сверху, ты отмечен тёмной точкой">
    ...
    ${river}<ellipse cx="${layout.pond.x}" cy="${layout.pond.z}" rx="${layout.pond.rx}" ry="${layout.pond.rz}" fill="#78b8c1"/>${bridges}
    ${layout.routes.map((route) => `<polyline points="${route.map((p) => `${p.x},${p.z}`).join(' ')}" fill="none" stroke="#f5e8c6" stroke-width=".75" stroke-linecap="round"/>`).join('')}
    ${layout.homes.map((entry) => `<rect x="${entry.x - 1}" y="${entry.z - .8}" width="2" height="1.6" rx=".3" fill="#d69a7e" transform="rotate(${entry.rotation * 180 / Math.PI} ${entry.x} ${entry.z})"/>`).join('')}
    ...
    ${layout.region === 'home' ? `<rect x="${world.house.x - 1}" y="${world.house.z - 1}" width="2" height="2" rx=".25" fill="#bd765f"/>` : ''}
    ...
    <p>Тёмная точка — ты. Коричневый причал — лодка. Маленькие дома — соседи; голубая линия — река.</p>
